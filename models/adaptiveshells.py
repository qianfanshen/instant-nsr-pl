...
@models.register('adaptiveshells')
class AdaptiveShellsModel(BaseModel):
    def setup(self):
    ...
    def update_step(self, epoch, global_step):
        ...
        def occ_eval_fn(x):
            sdf, features = self.geometry(x, with_grad=False)
            inv_s = features[..., 1:2].clip(1e-6, 1e6)
    ...
    def forward_(self, rays):
        ...
        positions_noised = positions + torch.normal(mean=0.0, std=1e-3, size=positions.shape, device=positions.device)
        ...
        feature = feature[..., 2:-3]
        alpha = self.get_alpha(sdf, inv_s, normal, t_dirs, dists)[...,None]
        #import pdb;pdb.set_trace()
        rgb = self.texture(feature, t_dirs, normal, positions)
        _, feature_noised = self.geometry(positions_noised, with_grad=False, with_feature=True)
        inv_s_noised = feature_noised[..., 1:2]
